...
export default function PrototypePurchasePanel({product,imageUrl,openHour,closeHour}:{product:Product;imageUrl?:string;openHour:number;closeHour:number}){
 ...
 return <div className="pb-card pb-product-panel">
  ...
  <button type="button" className="pb-pill pb-btn pb-purchase-add" onClick={add} disabled={!inStock||!hasDates} style={{width:'100%',marginTop:16}}><span>{!inStock?'Нет в наличии':!hasDates?'Выберите даты':'В корзину'}</span><span>→</span></button>
