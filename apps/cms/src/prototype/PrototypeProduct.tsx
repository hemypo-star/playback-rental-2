...
export default async function PrototypeProduct({id}:{id:number}){
 ...
 return <div className="pb-page"><section className="pb-container pb-grid" style={{paddingTop:14,paddingBottom:80}}>
  <div className="pb-product-left">
   ...
   {images.length>1&&<div className="pb-thumb-grid">{images.slice(1,5).map(url=><div key={url} className="pb-media" style={{aspectRatio:'1',borderRadius:16}}><Image src={url} alt={product.title} fill sizes="20vw" style={{objectFit:'cover'}}/></div>)}</div>}
   <div className="pb-card pb-product-copy"><div className="pb-kicker"><Link href="/" className="pb-link-hover">Главная</Link> / <Link href="/catalog" className="pb-link-hover">Каталог</Link>{category?.slug&&<> / <Link href={`/catalog/${category.slug}`} className="pb-link-hover">{category.name}</Link></>}</div><h1>{product.title}</h1>{product.subtitle&&<div style={{marginTop:10,fontSize:14,color:'var(--pb-sub)'}}>{product.subtitle}</div>}{product.description&&<p style={{margin:'20px 0 0',fontSize:16.5,lineHeight:1.5,color:'#2A2925',maxWidth:560}}>{product.description}</p>}</div>
   ...
   {related.length>0&&<div className="pb-related-grid">{related.map((p,i)=><PrototypeProductCard key={p.id} delay={i*60} product={{id:p.id,title:p.title,subtitle:p.subtitle,price:p.price,listingType:p.listingType,quantity:p.quantity,available:p.available,tag:p.tag,categoryName:categoryNameOf(p),imageUrl:mediaUrl(p.images?.[0]),unit:p.listingType==='rental'?'смена / 24 часа':'шт.'}}/>)}</div>}
  </div>
