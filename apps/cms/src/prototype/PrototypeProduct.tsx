...
export default async function PrototypeProduct({id}:{id:number}){
 ...
 return <div className="pb-page"><section className="pb-container pb-grid" style={{paddingTop:14,paddingBottom:80}}>
  <div className="pb-product-left">
   <div className="pb-media pb-product-hero">{main&&<Image src={main} alt={product.title} fill sizes="(min-width:1021px) 58vw,92vw" style={{objectFit:'cover'}}/>}<div className="pb-product-tags" style={{left:14,right:14,top:14}}><span className="pb-tag">{product.tag||category?.name||'Техника'}</span><span className={`pb-tag ${inStock?'pb-tag-light':'pb-tag-red'}`}>{inStock?'В наличии':'Нет в наличии'}</span></div></div>
   ...
   {kit.length>0&&<div className="pb-card" style={{padding:26}}><div className="pb-kicker">Что в комплекте</div><div style={{marginTop:12,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>{kit.map((k,i)=><div key={k.id??i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',borderRadius:14,background:'var(--pb-muted)'}}><span style={{width:24,height:24,borderRadius:'50%',background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10.5,fontWeight:700,color:'var(--pb-red)'}}>{i+1}</span><span style={{fontSize:14}}>{k.label}</span></div>)}</div></div>}
   {accessories.length>0&&<div className="pb-card" style={{padding:26}}><div className="pb-kicker">Совместимые аксессуары</div><div style={{marginTop:10}}>{accessories.map(a=><div key={a.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,padding:14,borderRadius:14}}><div><div style={{fontSize:15.5,fontWeight:500}}>{a.title}</div><div style={{marginTop:3,fontSize:12.5,color:'var(--pb-sub)'}}>{formatCurrency(a.price)}{a.listingType==='rental'?' / смена':''}</div></div><PrototypeAddButton product={{id:a.id,title:a.title,subtitle:a.subtitle,price:a.price,listingType:a.listingType,quantity:a.quantity,available:a.available,tag:a.tag,imageUrl:mediaUrl(a.images?.[0]),unit:a.listingType==='rental'?'смена':'шт.'}}/></div>)}</div></div>}
