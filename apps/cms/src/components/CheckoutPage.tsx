...
function CheckBox({ checked, onClick, label }: { checked: boolean; onClick: () => void; label: React.ReactNode }) {
  return (
    <div onClick={onClick} className="-mx-3 flex cursor-pointer items-start gap-3 rounded-2xl p-3 transition-colors duration-240 ease-expo hover:bg-muted">
      <span
        className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-[7px] text-[12px] font-bold transition-[background-color,color,transform] duration-240 ease-expo"
...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.success) {
        setErrors([result.error || 'Не удалось оформить заказ. Попробуйте ещё раз.'])
        return
      }
            ...
            <div className="mt-1">
              {cart.map((item) => {
                ...
                return (
                  <div
                    key={item.productId}
                    className="mx-[-14px] grid grid-cols-[92px_1fr_auto] items-start gap-4 rounded-[18px] px-3.5 py-4.5 transition-colors duration-240 ease-expo hover:bg-muted"
