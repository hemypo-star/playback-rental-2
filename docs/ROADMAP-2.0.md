...
**Stage 2 — Storefront port** (7 page groups, Local API data layer, cache-freshness handling) — **in progress, started 2026-08-20.** Page group 1 of 7 done (legal pages: `/privacy-policy`, `/user-agreement`, both dropped out of `proxy.ts`'s fallback matcher). Since every page needs it, this pass also built the shared chrome Stage 1 deliberately deferred — Navbar/Footer, the `lib/data/siteSettings.ts` Local API layer, and near-verbatim ports of CartBadge/AdminPanelLink/RentalDatePicker — so page groups 2–7 inherit working chrome instead of each needing to build it. `design-sync audit` run against the ported chrome and fixed to spec (20/21 transitions now have explicit `ease-*`/`duration-240`, one pre-existing gap left alone in shared `global.css` to avoid apps/web drift). Remaining: how-it-works/contact, index, catalog, product, promotions, checkout (in that order per the plan).
...
**Net:** of the ~7 estimated working days in this plan, Stage 0 (0.1/0.2/0.3) and Stage 1 have landed, and Stage 2 is underway (1 of 7 page groups, plus the shared chrome all the rest depend on) as of 2026-08-20; Stages 3–4 are still fully unstarted.
...
4. ~~**Stage 1 (shell + proxy reversal)**~~ — done 2026-08-20, verified live. **Stage 2 in progress**: legal pages + shared chrome (Navbar/Footer/data layer/islands) done; next up how-it-works/contact, then index, catalog, product, promotions, checkout last, per docs/PLAN-next-migration.md's own page order.
