...
**Net:** of the ~7 estimated working days in this plan, Stage 0 (0.1/0.2/0.3) and Stage 1 have landed, and Stage 2 is underway (6 of 7 page groups, plus the shared chrome all the rest depend on) as of 2026-08-20; Stages 3–4 are still fully unstarted.
...
4. ~~**Stage 1 (shell + proxy reversal)**~~ — done 2026-08-20, verified live. **Stage 2 in progress**: legal pages + shared chrome (Navbar/Footer/data layer/islands) done, how-it-works/contact done, homepage (`index`, the largest template in the plan at 297 lines) done 2026-08-20, catalog (index + `[slug]` + `CategorySidebar`) done 2026-08-20, product (`[id]` + `ProductPurchasePanel`/`QuantitySelector`) done 2026-08-20, promotions (`/promotions/:slug`, just the storefront route port — the collection/admin CRUD/page itself already existed from 2026-08-14) done 2026-08-20; **checkout is next up and last**, per docs/PLAN-next-migration.md's own page order — the plan itself flags it as needing the most care, since it's the only page group with a real mutation (`submitOrder()`, to become a Server Action on the Local API rather than a client-side REST POST).
